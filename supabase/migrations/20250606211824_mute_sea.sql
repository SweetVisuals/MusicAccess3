/*
  # User Wallets and Analytics System

  1. New Tables
    - `user_wallets`: Stores user balance information
    - `analytics_events`: Detailed analytics tracking system
    - `dashboard_metrics`: Pre-calculated metrics for dashboard display
  ...
  3. Functions
    - `update_user_wallet_updated_at`: Updates timestamp on wallet changes
    - `calculate_dashboard_metrics`: Calculates user metrics for different time periods
    - `create_user_wallet`: Automatically creates wallet for new users
    - `give_gem`: Handles gem transactions between users
*/
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE tablename = 'user_wallets' AND policyname = 'Users can view their own wallet'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'user_wallets' AND policyname = 'Users can update their own wallet'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'dashboard_metrics' AND policyname = 'Users can view their own dashboard metrics'
  ) THEN
